...
function paymentStatus($status) {
    if ($status == Status::PAYMENT_CREATED) {
        $array['text']  = 'Waiting For Payment';
        $array['class'] = 'warning';
    } elseif ($status == Status::PAYMENT_COMPLETED) {
        $array['text']  = 'Payment Completed';
        $array['class'] = 'success';
    } elseif ($status == Status::PAYMENT_WAITING) {
        $array['text']  = 'Waiting For Confirmation';
        $array['class'] = 'primary';
    } elseif ($status == Status::PAYMENT_REPORT_SENDER) {
        $array['text']  = 'Reported';
        $array['class'] = 'danger';
    } elseif ($status == Status::PAYMENT_REPORT_RECEIVER) {
        $array['text']  = 'Reported';
        $array['class'] = 'danger';
    } elseif ($status == Status::PAYMENT_CANCELLED) {
...
function levelCommission($investment) {
    $usr = $investment->user_id;
    ...
    while ($usr != "" || $usr != "0" || $i < $level) {
        ...
        $com = (($investment->amount * $commission->percent) / 100);
        $refer->save();

        if ($user->ref_by) {
            $commissionWallet = ReferralCommissionWallet::where('user_id', $user->ref_by)->where('currency_id', $investment->currency_id)->first();
            if ($commissionWallet) {
                $commissionWallet->amount += $com;
                $commissionWallet->save();
            } else {
                $wallet = new ReferralCommissionWallet();
                $wallet->user_id = $user->ref_by;
        ...
        RefLog::create([
            ...
            'description' => 'Level ' . $i . ' Referral Commission From ' . $user->fullname . ' after Complete Investment of ' . getAmount($investment->amount) . ' ' . $investment->currency->code,
        ]);
